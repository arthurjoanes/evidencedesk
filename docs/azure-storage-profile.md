...
A distinção entre exclusão corrente, versões e soft delete foi conferida na [documentação Microsoft de exclusão de blobs](https://learn.microsoft.com/en-us/azure/storage/blobs/storage-blob-delete-python). Os testes locais do adaptador continuam sendo doubles; não demonstram comunicação Azure ou expurgo físico de todas as cópias.
