...
Autenticação bearer ocorre antes de interpretar o corpo. O limite de 128.000 bytes também vale para corpo transmitido em partes. Campos extras, incluindo tenant ou endereço de fonte, são rejeitados. O cliente tem connect/write de dois segundos, pool de um segundo e read de quinze segundos; o servidor rejeita simultaneidade com 503 em vez de criar fila sem limite. Entradas E5 e pares de reranking acima de 512 tokens são recusados: rechunking é necessário, nunca corte orientado pelo gold.

Fontes dos limites: [serviço HTTP](../backend/src/evidencedesk/retrieval/model_service.py) e [cliente privado](../backend/src/evidencedesk/retrieval/http_client.py), conferidos em **22/09/2026**.
...
Fontes desta seção, conferidas em **22/09/2026**: [chunk-policy-tokenizers.json](../experiments/reports/chunk-policy-tokenizers.json) · [model-service-smoke-locked.json](../experiments/reports/model-service-smoke-locked.json) · [ml-final-build.json](../experiments/reports/ml-final-build.json).
