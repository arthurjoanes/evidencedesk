...
class RestoreCleanupTests(unittest.TestCase):
    def test_target_stop_failure_still_stops_source_and_saves_failed_evidence(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                ...
                patch("check_erasure_restore.scoped_running", return_value=[]),
                patch(
                    "check_erasure_restore.execute",
                    side_effect=[
                        subprocess.CalledProcessError(1, ["synthetic"]),
                        None,
                    ],
                ) as execute,
            ):
    ...
    def test_successful_stop_without_empty_inventory_cannot_approve_cleanup(self):
        with tempfile.TemporaryDirectory() as directory:
            output = Path(directory) / "attempt.json"
            with (
                patch("builtins.print"),
                patch(
                    "check_erasure_restore.scoped_running",
                    side_effect=[["a" * 12], ["a" * 12], [], []],
                ),
                patch(
                    "check_erasure_restore.execute",
